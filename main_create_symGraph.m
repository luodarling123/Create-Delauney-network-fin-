...
%% 预存数据
DataInform = zeros(num,14); %% 前6列为温度和Z，后4列为热阻，约化热阻，对流换热系数，约化对流换热系数，最后两列为约化表面积和约化体积
%% 存储位置
path = 'F:\FEM\GraphAnalysis2\CreateGraph\symGraph\12_\';
node_name = 'df_Random_net_xy'; 
bond_name = 'original_Network';
%% 开始画图
parfor i = 1 : num
    % 创建对称扭曲晶格
    [node_cor,bond_cor] = FunCreatesymGraph(n,bond,n_inside,Dis); 
    % 数据记录
...
writematrix(DataInform,...
    'F:\FEM\GraphAnalysis2\CreateGraph\symGraph\12_\12_symGraph.txt','Delimiter',' ');