...
% Main_for_Greedy_22.m

% Start MATLAB's parallel computing pool with the desired number of workers
%numWorkers = 28;  % Specify the number of workers (cores) you want to use
%parpool('local', numWorkers);

% Your MATLAB code here (parallelizable tasks)

% Shutdown the parallel computing pool when done
% delete(gcp('nocreate'));
...
file_dirname = '/data/home/duanfuqing/LTL/Matlab/Optimiztion/symGraph/22_';
node_subname = '/nodes/';
edge_subname = '/edges/';
n = 2505; %% 第n个，文件名为n-1
...
%% 调用非本文件夹函数
addpath(genpath('/data/home/duanfuqing/LTL/Matlab/Optimiztion/Function'));
...
Nor_h_i = h_avg_i/(20/(138.05*((121.6*121.6*2+121.6*2*3)/1e6)));


% 迭代信息
PerDelete = 1; %每次迭代删除的连接数
iteration = 400 ; % 总共迭代次数
...
writematrix(bond_cor,'GreedyNetworkDeletebondcor_22_edge2.txt');
writematrix(Size_info,'GreedyNetworkDeletebondcor_22_Size2.txt');
writematrix(h_info,'GreedyNetworkDeletebondcor_22_havg2.txt');
writematrix(Temp_info,'GreedyNetworkDeletebondcor_22_Temp2.txt');
writematrix(Delete_Edge,'GreedyNetworkDeletebondcor_22_Delete2.txt');